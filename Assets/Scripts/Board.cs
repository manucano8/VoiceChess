...
public class Board : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        spawnAllPieces();
        //posicionamos todas las piezas
        positionAllPieces();
    }

    //devuelve la pieza que se encuentra en la posicion dada
    public Piece choosePiece(int x, int y){
            //comprueba si existe una pieza en la posicion
            if (pieces[x, y] != null){
                //comprueba si la pieza seleccionada es igual a la del jugador que tiene el turno
                if ((pieces[x, y].team == 0 && whiteTurn) || (pieces[x, y].team == 1 && !whiteTurn)){
                    ...
                    //guarda todos los posibles movimientos de la pieza actual
                    availableMoves = currentlySelected.getAvailableMoves(ref pieces, square_count_x, square_count_y);
                    //guarda todos los posibles movimientos especiales 
                    specialMove = currentlySelected.getSpecialMoves(ref pieces, ref moveList, ref availableMoves);
                    //comprobacion de posible jaque
                    preventCheck();
                    //muestra las casillas con los movimientos disponibles
                    highlightMoves();
                }
            }
    ...
    //mueve la pieza seleccionada a la posicion dada
    public void moveChosen(Piece currentlySelected, int x, int y){
        //comprueba si se ha seleccionado alguna pieza
        if (currentlySelected != null){
            ...
            //En caso de que el movimiento no sea valido, se devolvera la pieza a la posicion anterior
            if (!validMove){
                if (currentlySelected.type == PieceType.Rey) {
                    if (currentlySelected.team == 0){
                        currentlySelected.setPosition(getSquareCenter(previousPosition.x - 2, previousPosition.y));
                    }
                    else {
                        currentlySelected.setPosition(getSquareCenter(previousPosition.x + 2, previousPosition.y));
                    }
    ...
    //posiciona la pieza en la posicion dada
    private void positionOnePiece(int x, int y, bool force = false){
        ...
        if (pieces[x, y].type == PieceType.Rey) {
            if (pieces[x, y].team == 0) {
                pieces[x, y].setPosition(getSquareCenter(x - 2, y), force);
            }
            else {
                pieces[x, y].setPosition(getSquareCenter(x + 2, y), force);
            }
    ...
    //cambia el color de las casillas de los posibles movimientos
    ...
    //borra el color de las casillas de los posibles movimientos
    ...
    private void processSpecialMove() {
        //se ha utilizado el movimiento pasante
        if (specialMove == SpecialMove.Pasante){
            ...
            //comprueba que ambos peones se encuentran en la misma fila
            if (miPeon.currentX == peonEnemigo.currentX){
                //comprueba que el peon enemigo se encuentra a nuestra izquierda o derecha
        ...
        //se ha utilizado el movimiento enroque
        ...
        ////se ha utilizado el movimiento promocion
        if (specialMove == SpecialMove.Promocion) {
            Vector2Int[] lastMove = moveList[moveList.Count - 1];
            Piece peon = pieces[lastMove[1].x, lastMove[1].y];
            //comprueba que es un peon
            if (peon.type == PieceType.Peon){
                //comprueba que ha llegado a la casilla final
    ...
    //comprueba si existe un posible jaque
    ...
    //simula todos los movimientos de una pieza
    ...
    private bool MoveTo(Piece p, int x, int y){
        ...
        if (pieces[x, y] != null){
            ...
            if (auxp.team == 0){
                ...
                //elimina la pieza y la mueve fuera del tablero
                deadWhite.Add(auxp);
